...
# CONFIGURATION
# ============================================================================
RELEASE_SHA256="2609342C91DBBEB21F3E5BD4E61BDB357545909F2533C6D2C25ED208397D63F1"
...
ACTUAL_SHA256=$(sha256sum "$ARTIFACT_PATH" | cut -d' ' -f1 | tr '[:upper:]' '[:lower:]')
...
if [ "$DRY_RUN" = true ] || [ "$PREFLIGHT" = true ]; then
    ...
    if [ ! -d "$STAGE_DIR/backend" ]; then
        fail "Artifact structure invalid — missing backend/ directory"
    fi

    # Verify required deploy files exist in artifact
    for file in "${DEPLOY_FILES[@]}"; do
