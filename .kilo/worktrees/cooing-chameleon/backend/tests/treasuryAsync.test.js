...
describe('3B.2-F — Treasury branch isolation', () => {
  let app;
  let dir;
  let tokenA;
  let tokenB;
  ...
  const branchUsers = { users: [
    {
      id: 'u-ba', username: 'branchA', password: hash('Pass#123'), role: 'Owner',
      fullName: 'Branch A', tenantIds: ['corp-a'], tenantRoles: { 'corp-a': 'Owner' },
      branchId: 'branch-a',
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
    },
    {
      id: 'u-bb', username: 'branchB', password: hash('Pass#123'), role: 'Owner',
      fullName: 'Branch B', tenantIds: ['corp-b'], tenantRoles: { 'corp-b': 'Owner' },
      branchId: 'branch-b',
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
    }
  ]};

  const branchTreasury = {
    entries: [
      { id: 'TX-BA-1', type: 'in', amount: 500, balance: 500, desc: 'A cash in', method: 'cash', user: 'branchA', tenantId: 'corp-a', branchId: 'branch-a', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      { id: 'TX-BB-1', type: 'out', amount: 100, balance: 400, desc: 'B expense', method: 'card', user: 'branchB', tenantId: 'corp-b', branchId: 'branch-b', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
      { id: 'TX-LEG-BRANCH', type: 'in', amount: 50, balance: 50, desc: 'Legacy no branch', method: 'cash', user: 'legacy', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
    ]
  };
  ...
  beforeAll(async () => {
    ...
    dir = makeTempDataDir('treasury-branch');
    seed(dir, 'companies', companies);
    seed(dir, 'users', branchUsers);
    seed(dir, 'treasury', branchTreasury);
    const s = await startServer(dir, { AUTH_REQUIRED: 'true', ENABLE_BRANCH_ISOLATION: 'true' });
    app = s.app;
    tokenA = await loginAs(app, 'branchA', 'corp-a');
    tokenB = await loginAs(app, 'branchB', 'corp-b');
  });
  ...
  describe('create stamps trusted branch and rejects forged branch', () => {
    test('create stamps the trusted branchId from branchStore', async () => {
      const res = await post('/api/v1/treasury', { id: 'TX-BA-NEW', type: 'in', amount: 100, balance: 600, desc: 'A new deposit' }, tokenA);
      expect(res.statusCode).toBe(201);
      expect(res.body.data.branchId).toBe('branch-a');
      const onDisk = readStore(dir, 'treasury');
      expect(onDisk.entries.find(e => e.id === 'TX-BA-NEW').branchId).toBe('branch-a');
    });

    test('client-supplied forged branch B is REJECTED (403)', async () => {
      const before = JSON.stringify(readStore(dir, 'treasury'));
      const res = await post('/api/v1/treasury', { id: 'TX-FORGED', type: 'in', amount: 10, branchId: 'branch-b' }, tokenA);
      expect(res.statusCode).toBe(403);
      expect(JSON.stringify(readStore(dir, 'treasury'))).toBe(before);
    });
  });

  describe('list/get/stats are branch-scoped', () => {
    test('list: branch A sees own + legacy only; branch B sees own + legacy only', async () => {
      const a = await get('/api/v1/treasury', tokenA);
      const b = await get('/api/v1/treasury', tokenB);
      expect(a.statusCode).toBe(200);
      expect(b.statusCode).toBe(200);
      const idsA = a.body.data.entries.map(e => e.id);
      const idsB = b.body.data.entries.map(e => e.id);
      expect(idsA).toContain('TX-BA-1');
      expect(idsA).toContain('TX-LEG-BRANCH');
      expect(idsA).not.toContain('TX-BB-1');
      expect(idsB).toContain('TX-BB-1');
      expect(idsB).toContain('TX-LEG-BRANCH');
      expect(idsB).not.toContain('TX-BA-1');
    });

    test('getById: own 200, legacy 200, foreign 404', async () => {
      const own = await get('/api/v1/treasury/TX-BA-1', tokenA);
      expect(own.statusCode).toBe(200);
      const legacy = await get('/api/v1/treasury/TX-LEG-BRANCH', tokenA);
      expect(legacy.statusCode).toBe(200);
      const foreign = await get('/api/v1/treasury/TX-BB-1', tokenA);
      expect(foreign.statusCode).toBe(404);
    });
    ...
    test('stats are branch scoped', async () => {
      ...
      expect(a.statusCode).toBe(200);
      expect(b.statusCode).toBe(200);
      // Branch A sees TX-BA-1 + TX-LEG-BRANCH + TX-BA-NEW = 3 entries
      expect(a.body.data.count).toBe(3);
      // Branch B sees TX-BB-1 + TX-LEG-BRANCH = 2 entries
      expect(b.body.data.count).toBe(2);
    });
  });

  describe('cross-branch mutation blocked', () => {
    test('cross-branch update blocked (404) + other branch record survives', async () => {
      const before = JSON.stringify(readStore(dir, 'treasury'));
      const res = await put('/api/v1/treasury/TX-BB-1', { amount: 9999, desc: 'Hijack' }, tokenA);
      expect(res.statusCode).toBe(404);
      expect(JSON.stringify(readStore(dir, 'treasury'))).toBe(before);
    });

    test('cross-branch delete blocked (404) + other branch record survives', async () => {
      const res = await del('/api/v1/treasury/TX-BB-1', tokenA);
      expect(res.statusCode).toBe(404);
      const onDisk = readStore(dir, 'treasury');
      expect(onDisk.entries.some(e => e.id === 'TX-BB-1')).toBe(true);
    });

    test('own update works', async () => {
      const res = await put('/api/v1/treasury/TX-BA-1', { desc: 'Branch A updated' }, tokenA);
      expect(res.statusCode).toBe(200);
      expect(res.body.data.desc).toBe('Branch A updated');
      expect(res.body.data.branchId).toBe('branch-a');
    });
  });

  describe('legacy records without branchId are visible to all branches', () => {
    test('legacy entry (no branchId) is visible to branch A and branch B', async () => {
      const a = await get('/api/v1/treasury/TX-LEG-BRANCH', tokenA);
      const b = await get('/api/v1/treasury/TX-LEG-BRANCH', tokenB);
      expect(a.statusCode).toBe(200);
      expect(b.statusCode).toBe(200);
    });
  });
