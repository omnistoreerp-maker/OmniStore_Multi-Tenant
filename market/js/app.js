(function () {
  ...
  async function pageHome() {
    ...
    const [cats, prods] = await Promise.all([
      window.MK_API.categories().catch(() => { loadError = true; return { categories: [] }; }),
      window.MK_API.products({ limit: 8 }).catch(() => { loadError = true; return { products: [] }; })
    ]);
    ...
    if (loadError) {
      html = '<div class="mk-banner error">' + esc(t('error_generic')) + '</div><p><button class="mk-btn secondary" id="mk-retry" type="button">' + esc(t('gh_retry')) + '</button></p>' + html;
    }
    setApp(html);
  ...
  async function pageCatalog() {
    ...
    let listError = false;
    const [cats, prods] = await Promise.all([
      window.MK_API.categories().catch(() => { listError = true; return { categories: [] }; }),
      window.MK_API.products({ categoryId: cat, search: q, sortBy: sort === 'price_asc' ? 'price' : sort === 'price_desc' ? 'price' : 'name', sortOrder: sort === 'price_desc' ? 'desc' : 'asc', limit: 100 }).catch(() => { listError = true; return { products: [] }; })
    ]);
    ...
    if (listError) {
      html += '<div class="mk-banner error">' + esc(t('error_generic')) + '</div><p><button class="mk-btn secondary" id="mk-list-retry" type="button">' + esc(t('gh_retry')) + '</button></p>';
    } else if (!(prods.products || []).length) {
      html += emptyState('package', t('no_products'), null, '<a class="mk-btn" href="#/catalog">' + esc(t('empty_browse_catalog')) + '</a>');
    }
    setApp(html);

    const listRetry = document.getElementById('mk-list-retry');
    if (listRetry) listRetry.addEventListener('click', () => render());
