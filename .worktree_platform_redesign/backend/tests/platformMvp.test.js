...
describe('Platform MVP — static surface', () => {
  test('platform.html is the public home with honest minimal layout', () => {
    ...
    expect(html).toContain('OmniStore Platform');
  ...
  test('Marketplace card does not link to a dead /market.html route', () => {
    const html = repoFile('platform.html');
    expect(html).not.toContain('/market.html');
  });
...
describe('Platform MVP — `/` serves Platform Home and public APIs', () => {
  ...
  test('GET / serves the Platform Home markup, not the ERP dashboard', async () => {
    ...
    expect(res.text).toContain('OmniStore Platform');
  ...
  test('GET /platform.html and /business.html are served', async () => {
    ...
    expect(p.text).toContain('OmniStore Platform');
  ...
  test('Platform Home switches language and persists choice', async () => {
    const js = repoFile('platform/platform.js');
    expect(js).toContain('localStorage');
    expect(js).toContain('omnistore_platform_lang');
  });

  test('Platform Home does not expose tenant count as activity', async () => {
    const js = repoFile('platform/platform.js');
    expect(js).toContain('renderActivity()');
    expect(js).not.toContain("label === 'Tenants'");
  });
});
