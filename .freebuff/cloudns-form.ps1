...
javascript:void((function(){
  ...
  document.querySelectorAll('input,select,textarea').forEach(function(el){
    inputs.push({tag:el.tagName,name:el.name,type:el.type,placeholder:el.placeholder,id:el.id});
  });
  var imgs = [];
  document.querySelectorAll('img').forEach(function(img){
    if (img.src && (img.src.indexOf('captcha') >= 0 || img.src.indexOf('securimage') >= 0 || img.src.indexOf('recaptcha') >= 0)) {
      imgs.push(img.src);
    }
  });
  document.title = JSON.stringify({inputs:inputs,captcha:imgs});
})())
'@
                $vp.SetValue($js)
