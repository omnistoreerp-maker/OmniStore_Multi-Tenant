...
# I have the zone ID from the URL: 7154f110196b9350716a79dbc2401aef
...
foreach ($el in $allElements) {
    try {
        if ($el.Current.ClassName -eq 'OmniboxViewViews') {
            ...
            if ($vp) {
                $url = "https://dash.cloudflare.com/7154f110196b9350716a79dbc2401aef/omnistore.eu.org/dns"
                $vp.SetValue($url)
                Start-Sleep -Milliseconds 300
                [System.Windows.Forms.SendKeys]::SendWait("{ENTER}")
                Write-Host "Navigating to DNS..."
            }
            break
        }
    } catch {}
