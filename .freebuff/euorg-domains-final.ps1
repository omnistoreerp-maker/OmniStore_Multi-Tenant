...
foreach ($el in $allElements) {
    try {
        ...
        if ($name -eq 'Domains') {
            try { $el.SetFocus(); Start-Sleep -Milliseconds 200; $el.Click(); Write-Host "Clicked Domains" } catch {
...
foreach ($el in $chrome.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)) {
    try {
        if ($el.Current.ClassName -eq 'OmniboxViewViews') {
            ...
            if ($vp) {
                $js = 'javascript:void(document.title="DOMAINS:"+document.body.innerText.substring(0,2000))'
                $vp.SetValue($js)
