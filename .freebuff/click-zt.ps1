...
# Find "Zero Trust" hyperlink
$allElements = $chrome.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)
foreach ($el in $allElements) {
    try {
        $ENAME = $el.Current.Name
        $ECONTROL = $el.Current.ControlType.ProgrammaticName
        if ($ECONTROL -eq 'ControlType.Hyperlink' -and $ENAME -eq 'Zero Trust') {
            Write-Output "Found Zero Trust hyperlink"
...
# Read page
Write-Output "=== AFTER ZERO TRUST CLICK ==="
