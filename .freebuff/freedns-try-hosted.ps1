...
# Now read the page to find form fields
$chromeWindows = $root.FindAll([System.Windows.Automation.TreeScope]::Children, [System.Windows.Automation.Condition]::TrueCondition)
foreach ($w in $chromeWindows) {
    try {
        if ($w.Current.ClassName -eq 'Chrome_WidgetWin_1' -and $w.Current.Name -match 'Chrome') {
...
foreach ($el in $allElements) {
    try {
        ...
        if ($name -and $name.Length -gt 1 -and $name.Length -lt 300) {
            if ($name -match 'subdomain|type|domain|destination|captcha|save|host|CNAME|A |MX|TXT|adding|error|restricted') {
                $found += "[$type] $name"
            }
        }
    } catch {}
}
Write-Host "Form elements: $($found.Count)"
$found
