...
foreach ($el in $allElements) {
    try {
        if ($el.Current.ClassName -eq 'OmniboxViewViews') {
            ...
            if ($vp) {
                $vp.SetValue("https://dash.cloudflare.com/?to=/:account/dns")
                Start-Sleep -Milliseconds 300
                [System.Windows.Forms.SendKeys]::SendWait("{ENTER}")
                Write-Host "Navigating to Cloudflare DNS..."
            }
            break
        }
    } catch {}
...
foreach ($el in $allElements) {
    try {
        ...
        if ($name -and $name.Length -gt 2 -and $name.Length -lt 200) {
            if ($name -match 'omnistore|eu\.org|nameserver|NS |dns|cloudflare\.com') {
                $found += "[$type] $name"
            }
        }
    } catch {}
