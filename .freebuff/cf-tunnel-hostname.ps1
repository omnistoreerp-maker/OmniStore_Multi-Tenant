...
# Navigate to Cloudflare Zero Trust Tunnels
...
foreach ($el in $allElements) {
    try {
        if ($el.Current.ClassName -eq 'OmniboxViewViews') {
            ...
            if ($vp) {
                $vp.SetValue("https://one.dash.cloudflare.com/?to=/:account/networks/tunnels")
                Start-Sleep -Milliseconds 300
                [System.Windows.Forms.SendKeys]::SendWait("{ENTER}")
                Write-Host "Navigating to Zero Trust Tunnels..."
            }
            break
        }
    } catch {}
