...
# Use JS to find the DNS Records link href
...
javascript:void((function(){
  ...
  links.forEach(function(a){
    if (a.textContent.indexOf('DNS Records') >= 0 || a.href.indexOf('DNS') >= 0) {
      dnsLinks.push({text:a.textContent.trim().substring(0,60), href:a.href});
    }
  });
  document.title = JSON.stringify(dnsLinks);
})())
'@
                $vp.SetValue($js)
