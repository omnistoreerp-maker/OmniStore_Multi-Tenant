...
const TERMINAL_ORDER_STATUSES = Object.freeze(['cancelled', 'terminated', 'expired']);

async function applyPaymentResult({ orderId, tenantContext, paymentRef, amount, status, meta } = {}) {
  const trustedTid = _trustedTenantId(tenantContext);
  // Fail closed: payment mutation requires an established trusted tenant.
  if (!trustedTid) return { error: 'tenant context is required' };
  ...
  try {
    ...
    // Tenant binding: order must belong to the trusted tenant (fail closed).
    if (String(order.tenantId) !== trustedTid) return { error: 'Order not found' };
    ...
    // Terminal / non-pending orders must not absorb payment callbacks.
    // Preserves ORDER_TRANSITIONS: only `pending` may become `paid`.
    if (TERMINAL_ORDER_STATUSES.indexOf(order.status) !== -1) {
      return {
        error: 'Order is ' + order.status + '; payment results cannot mutate this order',
        order: _publicOrder(order)
      };
    }
    if (order.status !== 'pending') {
      return {
        error: 'Order must be pending before payment (current: ' + order.status + ')',
        order: _publicOrder(order)
      };
    }
    ...
    // Pending → paid only (state-machine invariant; never assign blindly).
