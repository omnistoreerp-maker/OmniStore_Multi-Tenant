...
// Gateway webhook — signature is enforced upstream by
// verifyPaymentsWebhookSignature (fail-closed). Idempotent by paymentRef.
async function paymentWebhook(req, res) {
  try {
    ...
    // Tenant binding: only the validated market tenant (requireMarketTenant
    // → req.marketTenant) may drive payment mutation. Body tenantId is ignored.
    // Fail closed when no trusted tenant context is established.
    const tenantContext = _tenantContext(req);
    if (!tenantContext || !tenantContext.tenantId) {
      return error(res, 'tenant context is required', 400);
    }
    const result = await orderService.applyPaymentResult({
