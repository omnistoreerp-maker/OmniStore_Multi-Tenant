...
// sandbox. The pinned contract: exactly one backend mutation (creating the
// opposite entry), strictly after the final confirmation; backend failure
// keeps the local mirror untouched; cancel at any stage mutates nothing.
...
function buildSandbox(opts = {}) {
  ...
  vm.runInContext(extractFunction('reverseTreasuryEntry'), context, { filename: 'DigiTronics_v5.html-reverseTreasuryEntry.js' });
  return { context, db, calls };
}

const ADMIN = { username: 'admin-1', role: 'Admin' }; // PIN required for non-Owner

describe('frontend treasury reversal backend-mode gate (real DigiTronics_v5.html functions)', () => {
