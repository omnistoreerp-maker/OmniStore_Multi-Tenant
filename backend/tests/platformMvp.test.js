...
describe('Platform MVP — hardened public onboarding', () => {
  ...
  test('valid onboarding creates the company, owner with server-owned role, and audits the public actor', async () => {
    ...
    expect(created.provisionedBy).toBe('platform-public-onboarding');
  ...
  test('duplicate company id returns a generic 409 with no enumeration hint', async () => {
    const res = await request(server.app)
      .post('/api/v1/platform-public/onboarding/provision')
  ...
  test('duplicate admin username returns the SAME generic 409 (no username enumeration)', async () => {
    const res = await request(server.app)
      .post('/api/v1/platform-public/onboarding/provision')
